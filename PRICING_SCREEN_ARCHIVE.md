# Archived: Quiz Pricing Screen (Foundation / Pod)

The full post-quiz pricing page (screen 14) that showed the matched tier with
founding-member pricing, benefits, testimonial, and the Foundation/Pod toggle.
Removed from the live flow on 2026-07-11 when the quiz began redirecting to
/landing-pad — preserved here for post-launch reuse.

**To restore the complete original quiz page with this screen:**
```
git show 2b65b6e:app/quiz/page.tsx
```

Full original file below (the pricing screen is `case 14`, plus its helpers:
Q4_EMPATHY_VAR, Q4_TESTIMONIAL, CheckIcon, CommunityDropdown, and the
qz-plan/qz-feat-row/qz-community-btn/qz-toggle-link styles).
